...
<br>
<br>
<img src="./img/JAVA/3.png" alt="1" style="zoom:50%;" />
<br>
<br>
...
보통은 Checked Exception 이더라도, 추상화 수준에 따라서 그 수준에 맞는 예외를 던지는 것이 좋다. Checked Exception 이더라도 실제 구현상의 추상화 레벨에 맞는 Unchecked Exception 으로 번역해서 예외를 던지면 좋다. 또한, 롤백이 수행되어야 하는 예외일 경우에도 이렇게 Unchecked Exception 으로 예외를 번역한다면 좋다. (Effective Java Item 71, Item 73)
<br> 
Checked Exception 이 꼭 나쁜 것만은 아니다. 어떤 물건을 주문시 주문은 성공했지만, 결제가 실패한 경우가 있다. 이 경우 catch 구문 내에서는주문 대기중으로 상태를 저장하고 입금이 완료될 때 까지 3일 동안을 주문 대기중 처리한다고 할때 이런 경우에 Checked Exception 을 사용할수도 있다. 때에 맞춰서 명확하게 예외를 처리해야할 정형적인 상황이라면 Checked Exception 을 사용할 수 있다.
<br>
...
예제는 [querydsl,jpa](https://github.com/soon-good/study-querydsl-jpa/tree/develop/codes/transactional_study/src/test/java/io/study/transactional_study/commit_rollback) 에 조금 정리해두었다. 

이번 문서에서는 여기까지만 정리하기로 했다. 너무 깊이 들어가면, 자바에 관련된 내용만 총정리해둔 문서를 읽는데에 시간이 많이 걸린다. 의식의 흐름도 끊기는것이 단점이 될 것이라고 생각했다. 테스트 케이스가 3개 정도 더 있는데 이것과 관련해서는 데이터베이스, 트랜잭셔널, DB라이브러리 정리문서 리포지터리 (jpa-querydsl-sql) 이라는 리포지터리에 정리해 두기로 했다.
<br>
<br>
나 혼자 편하려면, 예전에 정리해둔 링크를 여기에 정리해두려고 했지만, 이 문서는 구글 검색 봇이 수집하게 되어서 예전에 없는 시간 쪼개서 지저분하게 정리한 내용들까지 여기에 링크를 남겨두지는 못했다. 따로 텍스트 문서에 링크를 정리해두었지만, 텍스트 파일이 한둘이 아니라서.... ㅋㅋ 이번주 꼭 까먹지 말고 정리를 해야 할텐데 까먹지 않을지 걱정이다.
...
### etc
**2022.03.08 새벽**<br>
요즘 바람 앞의 등불이 된 기분이다.으으... 금요일 오후까지 이 문서 얼른 마무리 짓도록 해야겠다. 이 문서에는 흑마법이 깃들어있는건 아니겠지? 제발 아니길!!<br>
<br>
<br>
